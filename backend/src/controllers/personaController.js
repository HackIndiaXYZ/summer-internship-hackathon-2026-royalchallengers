...
const setupProfile = async (req, res) => {
  const { user_id, age, gender, health_conditions, health_goals, dietary_preferences } = req.body;
  try {
    const result = await query(
      `INSERT INTO personas (user_id, age, gender, health_conditions, health_goals, dietary_preferences)
       ...
       updated_at = NOW()
       RETURNING *`,
      [user_id, age, gender, health_conditions, health_goals, dietary_preferences]
    );
